...
import { Button, message, Card, Statistic, Row, Col, Table, Select, Space } from "antd";
...
export default function EmployeeDashboard() {
  ...
  // Kendi departmanına göre çözümlenmiş mesai kuralı (departmana özelse o,
  // yoksa genel kural) - başlıkta PLANLANAN Giriş/Çıkış saatini göstermek
  // için. Gerçek attendance kaydı DEĞİL.
  const fetchMySchedule = useCallback(async () => {
    try {
      const res = await api.get("/work-schedules/mine");
      setMySchedule(res.data);
    } catch (err) {
      // sessizce geç - böyle bir kural yoksa başlıkta sadece gösterilmez
      setMySchedule(null);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    fetchStatus();
    fetchStats();
  }, [fetchAttendance, fetchMySchedule, fetchStatus, fetchStats]);
